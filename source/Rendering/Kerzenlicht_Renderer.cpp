...
Kerzenlicht_Renderer::Kerzenlicht_Renderer() {
	...
	for (uint16 x = 0; x < resolution.x; x++) {
		for (uint16 y = 0; y < resolution.y; y++) {
			f_drawPixel(x, y, vec4((float)x / resolution.x, 0.0f, (float)y / resolution.y, 1.0f));
		}
	}
}

void Kerzenlicht_Renderer::f_resize(const uint16& i_x, const uint16& i_y) {
	delete[] render_data;
	delete[] display_data;
	resolution = uvec2(i_x, i_y);
	render_data = new float[resolution.x * resolution.y * 4];
	display_data = new float[0];
}

void Kerzenlicht_Renderer::f_drawPixel(const uint16& i_x, const uint16& i_y, const fvec4& i_color) {
...
void Kerzenlicht_Renderer::f_drawPixel(const uvec2& i_pixel, const fvec4& i_color) {
